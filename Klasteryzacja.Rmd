...
Teraz możemy przygotować prostą wizualizację jednego kanału (np. ultra blue; B1)
oraz poligonu.

```{r}
# wizualizacja
plot(landsat[[1]]) # alternatywnie: plot(landsat[["B1"]])
plot(poly, add = TRUE)
```
...
scena satelitarna ma o wiele większy zasięg. W takiej sytuacji możemy dociąć rastry,
dzięki czemu przetwarzanie będzie szybsze, a finalna mapa zajmie mniej miejsca
...
Użyjemy najprostszego modelu [grupowania k-średnich](https://www.statsoft.pl/textbook/stcluan.html#k)
(*k-means*). Ten model wymaga jedynie, aby podać z góry liczbę klastrów/grup
...
trzeci do grupy 2, itd. Kolejnym etapem jest stworzenie mapy na podstawie
...
Następnie musimy przypisać nasze klastry w wektorze w odpowiednie miejsca,
...
W ostatnim kroku skopiujemy metadane obiektu `landsat`, ale tylko z jedną
...
`writeRaster()` i wczytać w innym programie.
...
Dane muszą być przedstawione w tzw. formie długiej (wiele wierszy), podczas gdy
standardowe funkcje do wizualizacji wymagają formy szerokiej (wiele kolumn).
...
Nie ma potrzeby przedstawiania wszystkich danych na wykresie. Wymaga to więcej 
